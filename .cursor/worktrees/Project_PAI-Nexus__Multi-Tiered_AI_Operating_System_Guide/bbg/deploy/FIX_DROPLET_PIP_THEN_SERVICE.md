...
If that still fails, try installing scipy without the only-binary constraint (sometimes a slightly different version has a wheel):
...
Wait for it to finish (can take a few minutes). If you see "Successfully installed ..." at the end, go to step 2.
...
You want **200**. Then open **http://157.245.7.40:5000** in a browser.

If the service is **failed**, read the log output from step 4 and see **FIX_VERTEX_API_KEY.md**.
