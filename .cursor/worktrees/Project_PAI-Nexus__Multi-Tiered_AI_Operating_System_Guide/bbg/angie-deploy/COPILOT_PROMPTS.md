...
## 3. Service not found (Unit angie.service could not be found)

Use when the systemd unit was never installed. Follow `deploy/INSTALL_SERVICE_ONLY.md` (scp angie.service, install under /etc/systemd/system/, daemon-reload, enable, start). Then verify with systemctl status angie and curl; if the service fails, check angie.log and FIX_VERTEX_API_KEY.md.
