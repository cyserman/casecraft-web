...
4. **API key errors / Vertex_ai_betaException / connection refused**
   - If logs show "API key not valid" or "Vertex_ai_betaException", Angie is crashing on LLM auth. Follow **FIX_VERTEX_API_KEY.md** (re-copy `.env`, set model to `gemini/gemini-2.5-flash`, restart).
   - Otherwise: on droplet `cat /root/.pai/.env` (ensure GEMINI_API_KEY is set). Re-copy from Dell: `scp ~/.pai/.env root@157.245.7.40:/root/.pai/.env`
